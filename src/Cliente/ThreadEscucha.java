...
public class ThreadEscucha extends Thread {
    ...
    /**
     * Inicio del hilo. Establecerá conexion con el servidor y se dispondrá a
     * escuchar los mensajes provenientes de este.
     */
    @Override
    public void run() {
        try {
    ...
    /**
     * Envía al Servidor la direccion que el cliente ha pulsado.
     *
     * @param dir Direccion a la que desea dirigirse el jugador.
     */
    public void enviarDireccion(Direccion dir) {
        try {
            //Envía los cambios de dirección al modelo para que efectúe los cálculos apropiados.
            PrintWriter out = new PrintWriter(this.socket.getOutputStream(), true);
            String direccion = dir.toString();
            out.println(ConstructorMensajes.dir(direccion)); //Construye un mensaje tipo DIR y lo envía.
        } catch (IOException ex) {
    ...
    /**
     * Cierra la conexión con el servidor y seguidamente cierra el programa.
     */
